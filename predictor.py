...
class Predictor(object):
    ...
    def store_data_relations(self):
        ...
        for l in lines:
            ...
            if i==0: 
                print l
                i = 1
            # rlist = [int(t) for t in l]
            # user, item, rating, timestamp = rlist[:]
            timestamp, user, item, rating = int(l[0]),int(l[1]),int(l[2]),float(l[3])
    ...
    def choose_some_items(self, item_ids, user_items, target_item, K):
        ...
        print len(item_ids), len(user_items), len(items)
